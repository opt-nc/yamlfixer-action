...
if [ $result -ne 0 ] ; then
  ...
  git checkout -b yamlfixer/$branch_name/$current_timestamp
  git add --all
  git commit -m 'Yamlfixer : fix yaml files '$YAML_FILE
  git push origin yamlfixer/$branch_name/$current_timestamp
  echo "INFO : create a pull request." ;
  curl  -u $USER:$TOKEN -H "Accept: application/vnd.github.v3+json" https://api.github.com/repos/opt-nc/$repository_name/pulls -d '{"head":"'yamlfixer/$branch_name/$current_timestamp'","base":"'$branch_name'", "title":"Fix yaml files '$YAML_FILE'"}'
else
