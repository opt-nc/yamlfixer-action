...
if [ $result -ne 0 ] ; then
  ...
  repository_name=$(git config --get remote.origin.url | sed -e 's/^git@.*:\([[:graph:]]*\).git/\1/')
  ...
  curl  -v -X POST -H "Accept: application/vnd.github.v3+json" https://api.github.com/repos/$repository_name/pulls \
