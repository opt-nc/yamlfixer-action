...
if [ $result -ne 0 ] ; then
  ...
  curl  -u $USER:$TOKEN -H "Accept: application/vnd.github.v3+json" https://api.github.com/repos/opt-nc/$repository_name/pulls -d '{"head":"'yamlfixer/$branch_name'","base":"'$branch_name'", "title":"Fix yaml files '$YAML_FILE'"}'
else
